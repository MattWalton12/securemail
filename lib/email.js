...
exports.list = function(user, start, amount, cb) {
  database.query("SELECT id, subject, from_email, UNIX_TIMESTAMP(date) AS date, marked_read FROM emails WHERE userid=? ORDER BY date DESC LIMIT ?, ?", [user.id, start, start+amount], function(err, emails) {
    cb(null, emails);
  });
}
...
exports.retrieve = function(user, id, cb) {
  ...
  if (id) {
    database.query("SELECT id, subject, from_email, UNIX_TIMESTAMP(date) AS date, encrypted_key FROM emails WHERE userid=? AND id=?", [user.id, id], function(err, email) {
      if (email && email[0]) {
        email = email[0];
        fs.readFile("email_store/" + user.id + "/" + email.id + ".sme", "binary", function(err, buf) {
          if (err) {
