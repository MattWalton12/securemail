...
exports.list = function(user, start, amount, type, cb) {
  database.query("SELECT id, subject, email, name, UNIX_TIMESTAMP(date) AS date, marked_read FROM emails WHERE userid=? AND type=? ORDER BY date DESC LIMIT ?, ?", [user.id, start, start+amount, type], function(err, emails) {
