...
exports.list = function(user, start, amount, cb) {
  database.query("SELECT id, subject, email, UNIX_TIMESTAMP(date) AS date, marked_read FROM emails WHERE userid=? ORDER BY date DESC LIMIT ?, ?", [user.id, start, start+amount], function(err, emails) {
    cb(null, emails);
  });
}
