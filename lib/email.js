...
exports.list = function(user, start, amount, search, type, cb) {
  function dbCallback(err, emails) {
    database.query("SELECT COUNT(*) AS count FROM emails WHERE marked_read=0 AND userid=? AND type=1", [user.id], function(err, res) {
  ...
  if (search.length > 0) {
    if (search.indexOf("tag:") > -1) {
      let tag = parseInt(search.split("tag:")[1])
      if (tag) {
        database.query("SELECT emails.id AS id, subject, email, name, UNIX_TIMESTAMP(date) AS date FROM emails, email_tags WHERE email_tags.tag_id=? AND emails.id=email_tags.email_id AND emails.type=? ORDER BY date DESC LIMIT ?, ?",
      ...
      search = "%" + search + "%"
      database.query("SELECT id, subject, email, name, UNIX_TIMESTAMP(date) AS date, marked_read FROM emails WHERE userid=? AND type=? AND (subject LIKE ? OR name LIKE ? OR email LIKE ?) ORDER BY date DESC LIMIT ?, ?",
        [user.id, type, search, search, search, start, start+amount], dbCallback)
    }
  } else {
    database.query("SELECT id, subject, email, name, UNIX_TIMESTAMP(date) AS date, marked_read FROM emails WHERE userid=? AND type=? ORDER BY date DESC LIMIT ?, ?", [user.id, type, start, start+amount], dbCallback)
  }
...
exports.addTag = function(userid, email, tag) {
  console.log("Adding tag ", userid, email, tag)
...
exports.removeTag = function(userid, email, tag) {
  console.log("Removing tag ", userid, email, tag)
  database.query("SELECT id FROM emails WHERE id=? AND userid=?", [email, userid], function(err, res) {
    if (res[0]) {
